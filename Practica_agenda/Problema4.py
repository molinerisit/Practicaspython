# Ejercicio 4 - Ordenar por fecha de nacimiento
# Enunciado: Ordenar los contactos del problema 3 por fecha de nacimiento antes de mostrarlos.
...
def mostrar_ordenados_por_fecha(archivo):
    archivo.seek(0)
    contactos = []
    try:
        while True:
    ...
    contactos.sort(key=lambda c: c.fechaNac)

    for contacto in contactos:
        print(f"{contacto.nombre} - {contacto.fechaNac}")
...
while True:
    ...
    if op == 1:
        ...
        mostrar_ordenados_por_fecha(archivo)
    elif op == 0:
        break
